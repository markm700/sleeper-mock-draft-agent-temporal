...
class SleeperCredentialManager:
    """Async client manager for the Sleeper public HTTP API.

    Uses a shared httpx.AsyncClient instance for connection pooling. The
    Sleeper API is read-only and does not require authentication.
    """
    ...
    def __init__(self, user: Optional[str] = None, league: Optional[str] = None):
        """Initialize the Sleeper API client for the given user and league."""
    ...
    async def _get_request(self, endpoint: str, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        Args:
            endpoint: API path segment, e.g. "user/{user_id}".
    ...
    async def get_user(self, username: Optional[str] = None) -> Dict[str, Any]:
        """Fetch user metadata by username or user ID."""
    ...
    async def get_user_leagues(
        ...
        """Fetch all leagues for a user across seasons, optionally filtered.

        When ``league_name`` is provided, only leagues whose ``name`` matches
        are returned for each season.
        """
        import datetime
...
def get_sleeper_client_manager(user: Optional[str] = None) -> SleeperCredentialManager:
    """Get or create the singleton SleeperCredentialManager instance."""
