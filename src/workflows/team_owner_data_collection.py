...
class TeamOwnerDataCollectionWorkflow:
    ...
    async def run(self, params: TeamOwnerDataCollectionWorkflowParams) -> Dict[str, Any]:
        ...
        workflow.logger.info(f"Get Team Owner Data Activity result: {team_owner_data['user_id'] if team_owner_data else 'No Team Owner found'}")
        ...
        rosters_seasons = [
            (season, league["league_id"]) for season, leagues in team_owner_data["user_leagues"].items() 
            for league in leagues if league.get("league_id")
        ] # returns season, league["league_id"]
        team_owner_roster_handlers = [
            workflow.execute_activity(
                get_team_owner_rosters,
                GetLeagueRosterParams(league_id=league_id, user_id=team_owner_data["user_id"]),
                start_to_close_timeout=timedelta(seconds=10),
                activity_id=(
                    f"activity-get_team_owner_rosters-{params.username}-{params.league_name}-{season}_season-{league_id}-{wf_hex}"
                ),
                retry_policy=activity_retry_policy,
            ) for season, league_id in rosters_seasons
        ]
        team_owner_rosters = await gather(*team_owner_roster_handlers)

        for (season, league_id), owner_roster in zip(rosters_seasons, team_owner_rosters):
            owner_league_rosters.append({
                "season": season,
                "league_id": league_id,
                "result": owner_roster["roster"] if "roster" in owner_roster else owner_roster.get("rosters", []),
            })
